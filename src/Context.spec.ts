...
describe("Context", () => {
  it("should create a proper arn", () => {
    const arn = Context.createInvokeFunctionArn(
      "us-east-1",
      "123456789012",
      "my-function"
    );
  ...
  it("should timeout properly", async () => {
    ...
    try {
      ...
      expect((err as Error).message).toEqual(
        "Task timed out after 1 second(s)"
      );
    }
  });

  it("should getRemainingTimeInMillis", async () => {
    expect.assertions(2);
    try {
      const context = new Context({
        startTime: Date.now(),
      ...
      setTimeout(() => {
        expect(context.getRemainingTimeInMillis()).toBeLessThan(500);
      }, 500);
      expect(context.getRemainingTimeInMillis()).toBeLessThan(1000);
      await new Promise((_, reject) => {
        context._reject = reject;
      });
    } catch {}
